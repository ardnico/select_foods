...
- [x] (2025-01-06 03:40Z) Preserve overlapping assignments when updating date ranges and added a regression test for the behavior.
- [ ] Run unit tests and manual walkthrough verifying period change, menu assignment, filtering, and ingredient aggregation (blocked: Swift toolchain unavailable in container).
...
- Observation: Date range updates now retain overlapping plan days while pruning out-of-range entries; this relies on calendar comparisons, so simulator verification should include cross-month spans.
  Evidence: Added `PlanStoreDateRangeTests` covering overlapping range adjustments.
...
- Minimal validation is in place for menus and ingredients with offline-only repositories. Date range changes now preserve existing assignments for overlapping days while dropping out-of-range ones. Need simulator run to confirm UI behavior and complete remaining acceptance steps; aggregation and date-range unit tests remain unexecuted until Swift toolchain is available.
